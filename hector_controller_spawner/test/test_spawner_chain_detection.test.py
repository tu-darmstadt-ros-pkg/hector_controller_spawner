...
def generate_test_description():
    return make_test_description(
        ...
        spawner_config="controller_spawner_chain_detection.yaml",
...
class TestControllerSpawner(unittest.TestCase):
    ...
    def test_controllers_loaded_and_activated(self):
        # vel_to_pos_controller is configured activate:false but sits in the chain of an active
        # controller, so the manager keeps it running - that is what this test pins down.
        assert_controller_states(
            self,
            self.node,
            {
